...
if($status==false){
  ...
  while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){
	 $view .= '<li class="scroll-item"><a href="#"><img src="img/ebook.png" class="icon"><time class="date">'.$result["indate"].'</time><span class="title">'.$result["book_name"].'</span><input type="hidden" value="'.$result["book_url"].'" class="url"><input type="hidden" value="'.$result["book_cmt"].'" class="cmt"><button class="button btn_detail" href="#">詳細</button></li>';
  }
}
?>
...
$(document).ready(function(){
	...
	// 詳細情報を表示する
	function showDetail(name, url, cmt){
		$("#name").val(name);
		$("#url").val(url);
		$("#cmt").val(cmt);
	}
...
<div class="detail">
	 ...
	 <button class="button btn_list">一覧へ戻る</button>
</div>
</div>
</div>
<footer class="footer">
